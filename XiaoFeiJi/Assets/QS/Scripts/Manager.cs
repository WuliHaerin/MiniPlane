...
public class Manager : MonoBehaviour {
	...
	public void CancelAd()
    {
		SetAdPanel(false);
		isCancelAd = true;
		GameObject.FindAnyObjectByType<Player>().Die();
    }

	public void Revive()
    {
		GameObject.FindAnyObjectByType<Player>().GetComponent<Player>().Revive();
    }
	...
	void Update () {
		...
		if (startGame && deadPlayer) {
			...
			if (!soundGameOver) {
				soundGameOver = true;
				SoundGameOver1();
				MakerScores(score, spriteScoreGO, new Vector2(0.0f, 0.0f), "MakerScore", spriteNumScoreGO);
				MakerScores(PlayerPrefs.GetInt ("HighScore"), spriteHighScoreGO, new Vector2(0.0f, 0.0f), "MakerHighScore", spriteNumScoreGO);
